...
function Cards({
  ...
  return (
    <div
      className="h-72 w-56 cursor-pointer rounded-lg bg-white p-2 dark:bg-slate-700 dark:text-slate-100"
      key={data.id}
      onClick={() => showProduct(data)}
    >
      <figure className="relative mb-2 h-4/5 w-full">
        <span className="absolute bottom-0 left-0 rounded-lg bg-white/60 px-2 text-lg font-bold lg:text-xl">
          {data.category.name}
        </span>
        <img
      ...
      <p className="flex justify-between">
        <span className="text-base font-light lg:text-lg">{data.title}</span>
